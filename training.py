...
flags.DEFINE_string('data_dir', "/ifs/loni/faculty/farshid/aimg/data/MPS", 'training data directory')
flags.DEFINE_string('modalities', 'flair_t1w_av45.masked', 'modalities to use in the training')
flags.DEFINE_string('logdir', "/scratch/hlan/experiment/pet_experiment/git_version", 'path of tensorboard log')
...
def main(args):
    ...
    num_subject = np.array(os.listdir(os.path.join(FLAGS.data_dir,'train')))
    modalities = FLAGS.modalities.split('_')
    num_samples = FLAGS.training_size
    shuf = np.arange(len(num_subject))
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for step in range(FLAGS.epoches*num_samples):
            pos = step % int(num_samples * 0.8//1)
            if pos == 0:
                np.random.shuffle(num_subject)
                avg_psnr = []
                avg_ssim = []
                avg_rmse = []
                train_set = num_subject[:int(num_samples * 0.8 // 1)]
                for i in range(int(num_samples * 0.8//1), num_samples):
                    val_list = list()
                    ...
                    test_pet = os.path.join(root, num_subject[i], '%s.nii.gz'%modalities[2])
            ...
            try:
                ...
                for num_modalities in modalities[:-1]:
                    ...
                    a = (a - np.mean(a[a != 0])) / np.std(a[a != 0])
                ...
                pet = (pet - np.mean(pet[pet != 0])) / np.std(pet[pet != 0])
